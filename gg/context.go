...
func (dc *Context) Width() int { return dc.width }

// Height returns the height of the image in pixels.
func (dc *Context) Height() int { return dc.height }
...
func (dc *Context) SetDash(dashes ...float64) { dc.dashes = dashes }

// SetDashOffset sets the initial offset into the dash pattern to use when
// stroking dashed paths.
func (dc *Context) SetDashOffset(offset float64) { dc.dashOffset = offset }

// SetLineWidth sets the lineWidth.
func (dc *Context) SetLineWidth(lineWidth float64) { dc.lineWidth = lineWidth }

// LineWidth returns the current line width
func (dc *Context) LineWidth() float64 { return dc.lineWidth }
...
func (dc *Context) ArcTo(x1, y1, x2, y2, radius float64) {
	...
	p1p0 := Point{X: p0.X - x1, Y: p0.Y - y1}
	...
	if cosPhi == 1 {
		...
		ep := Point{X: (p0.X + factorMax*p1p0.X), Y: (p0.Y + factorMax*p1p0.Y)}
		dc.LineTo(ep.X, ep.Y)
		return
	}
...
func (dc *Context) DrawRoundedRectangleExtended(x, y, w, h, tl, tr, br, bl float64) {
...
func (dc *Context) DrawQuadrilateral(x1, y1, x2, y2, x3, y3, x4, y4 float64) {
	dc.NewSubPath()
	dc.MoveTo(x1, y1)
	dc.LineTo(x2, y2)
	dc.LineTo(x3, y3)
